...
```{r}
make_plotly <- function(df, x, y, split = NULL, type = "scatter") {
    # require(lazyeval)
    se_line <- list(color = "rgba(7, 164, 181, 0.05)")
    se_fill <- "rgba(7, 164, 181, 0.2)"
    
    m <- loess(reformulate(x, y), data = df)
    
    # p <- plot_ly(x = interp(~x, x = as.name(x)))
    p <- plot_ly(x = reformulate(x))
    
    # split data if selected
    if (!is.null(split)) {
        split_frm <- reformulate(split)
        # df_split <- slice_rows(df, split) %>%
            # by_slice(map2, loess)
    } 
...
make_plotly(df, "duration_hypothermia", "lab.result", split = "order")

df_split <- slice_rows(df, "order") %>%
    do(m = loess(lab.result ~ duration_hypothermia, data = .))
            # by_slice(map2, .x = "duration_hypothermia", .y = "lab.result", .f = ~loess(.y ~ .x))
